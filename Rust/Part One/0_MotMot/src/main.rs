...
use world::{Coordinate, Map, MapGenerator, WorldMaths, WorldNavigate};
use motmot::MotMotNavigate;
use car::CarDriver;
...
impl SelfDrivingApp {
    ...
    fn draw_nodes(&self, painter: &Painter, rect: Rect, bounds: (f64, f64, f64, f64)) {
        for node in self.map.nodes() {
            ...
            let is_start = self.selected_start.as_deref() == Some(&node.id);
            let is_end = self.selected_end.as_deref() == Some(&node.id);
    ...
    fn handle_click(&mut self, pos: Pos2, rect: Rect, bounds: (f64, f64, f64, f64), right: bool) {
        for node in self.map.nodes() {
            ...
            if (dx * dx + dy * dy).sqrt() <= Self::CLICK_TOLERANCE {
                if right {
                    self.selected_end = Some(node.id.clone());
                } else {
                    self.selected_start = Some(node.id.clone());
                }
    ...
    fn refresh_planned_route(&mut self) {
        let (Some(start_id), Some(end_id)) = (&self.selected_start, &self.selected_end) else {
            self.planned_route = None;
            self.update_status();
            return;
        };
        let start = self.map.get_node_by_id(start_id);
        let end   = self.map.get_node_by_id(end_id);
    ...
    fn update_status(&mut self) {
        let start_name = self.selected_start.as_deref()
            .and_then(|id| self.map.get_node_by_id(id))
            .map(|n| n.name.clone());
        let end_name = self.selected_end.as_deref()
            .and_then(|id| self.map.get_node_by_id(id))
            .map(|n| n.name.clone());
    ...
    fn start_driving(&mut self) {
        let (Some(start_id), Some(end_id)) = (&self.selected_start, &self.selected_end) else { return };
        if self.planned_route.is_none() || self.is_driving { return; }

        let navigate = MotMotNavigate::new(Arc::clone(&self.map));

        let state = Arc::new(Mutex::new(CarState {
            position: self.map.get_node_by_id(start_id).map(|n| n.coordinate.clone()),
            bearing:  0.0,
            speed:    0.0,
            road_index: 0,
            is_active: true,
        ...
        let end   = end_id.clone();

        std::thread::spawn(move || {
            let mut car = CarDriver::new(navigate);
            car.start_driving_with_callback(&start, &end, stop_clone, move |position, bearing, speed, road_index, is_active| {
