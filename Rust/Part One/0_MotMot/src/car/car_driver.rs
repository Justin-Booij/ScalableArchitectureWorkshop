...
use rand::Rng;
use crate::world::{Coordinate, Road, WorldMaths};
use crate::motmot::{MotMotNavigate};
...
pub struct CarDriver {
    ...
    navigation: MotMotNavigate,
    rng: rand::rngs::ThreadRng,
}

impl CarDriver {
    pub fn new(navigation: MotMotNavigate) -> Self {
        Self {
        ...
        F: FnMut(Coordinate, f64, f64, usize, bool),
    {
        self.current_position = self
            .navigation
            .world_map()
            .get_node_by_id(start_id)
            .map(|n| n.coordinate.clone());

        self.is_active = true;

        if let Some(route) = self.navigation.navigate(start_id, dest_id) {
        ...
        if let Some(pos) = self.current_position.clone() {
            on_update(pos, self.current_bearing, self.current_speed, self.current_road_index, false);
        }
    }
        ...
        F: FnMut(Coordinate, f64, f64, usize, bool),
    {
        ...
        self.current_speed += self.navigation.get_speed_correction(self.current_road_index, self.current_speed);
        self.current_bearing += self.navigation.get_bearing_correction(self.current_road_index, self.current_bearing);
        ...
        while traveled_distance < distance {
            ...
            if let Some(pos) = self.current_position.clone() {
                let new_pos = WorldMaths::calculate_destination_point(&pos, self.current_bearing, step);
                self.current_position = Some(new_pos.clone());
                on_update(new_pos, self.current_bearing, self.current_speed, self.current_road_index, true);
            }
